...
const Watchlist = () => {
    ...
    const addItems = () => {
        ...
        if (selected === 'none') {
            alert('Currency not selected...Please Select the Cryptocurrency in the list')
        }
        if (selected !== 'none') {
            if (add) {
    ...
    return (
        <>
            ...
            <div className='dropdown'>
                ...
                {// eslint-disable-next-line
                    d.length !== 0 ? d.map((ele) => { list.push(ele.name) }) : (<p></p>)}

                <select className="form-select select form-select-sm" aria-label=".form-select-sm"
                ...
                <button className='button-add btn btn-primary' onClick={addItems}>Add</button>
            </div>
            <br /><br />
                    ...
                    <table className='table coin-table'>
                        <thead className='table-head'>
                            <tr>
                                ...
                                <th><strong>Volume for 24h in  &#8377;</strong></th>
                                <th><strong>Market Cap in  &#8377;</strong></th>
                                <th><strong>Visualize Price Change</strong></th>
                                    ...
                                    itemlist.map((ele, id) => {
                                        return (
                                            ...
                                            <tr>
                                                ...
                                                <td><Link to={`/watchlist/tracker/${ele.id}`}><i className="fas fa-chart-bar"></i></Link></td>
                                                <td>
                                                    <i className="fas fa-minus-circle x5" onClick={() => { deleteItem(ele.name) }}></i>
                                                </td>
                                            </tr>
